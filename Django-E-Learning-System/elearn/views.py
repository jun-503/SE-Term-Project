...
def lcreate_profile(request):
    ...
    if request.method == 'POST':
        ...
        avatar = request.FILES.get('avatar')  # Use get() to avoid MultiValueDictKeyError
        ...
        if avatar:
            ...
            # You can choose to set a default avatar or leave it empty
            profile = Profile.objects.create(
                user_id=user_id,
                first_name=first_name,
                last_name=last_name,
        ...
        # Retrieve existing profile data for the current user
        users = Profile.objects.filter(user_id=user_id)
        context = {'users': users}
        return render(request, 'dashboard/learner/create_profile.html', context)
